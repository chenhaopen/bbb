				...
				<el-row>
					<el-col :span="12">
						<el-form-item label="护工账号" prop="hugongzhanghao">
							<el-select
								...
								placeholder="请选择护工账号"
								@change="hugongzhanghaoChange">
								<el-option v-for="(item,index) in hugongzhanghaoLists" :label="item" :value="item">
								</el-option>
							</el-select>
						</el-form-item>
					</el-col>
					...
					<el-col :span="12">
						<el-form-item label="班次" prop="banci">
							<el-select
								class="list_sel"
								:disabled="!isAdd||disabledForm.banci?true:false"
								v-model="form.banci" 
								placeholder="请选择班次"
								>
								<el-option v-for="(item,index) in banciLists" :label="item"
									:value="item"
									>
								</el-option>
							</el-select>
					...
					<el-col :span="12">
						<el-form-item label="下班时间" prop="xiabanshijian">
							<el-date-picker
								class="list_date"
								v-model="form.xiabanshijian"
								format="YYYY-MM-DD HH:mm:ss"
								value-format="YYYY-MM-DD HH:mm:ss"
								type="datetime"
								:readonly="!isAdd||disabledForm.xiabanshijian?true:false"
								placeholder="请选择下班时间" />
						</el-form-item>
					</el-col>
					<el-col :span="12">
...
<script setup>
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		resetForm()
			form.value.riqi = context?.$toolUtil.getCurDate()
			form.value.shangbanshijian = context?.$toolUtil.getCurDateTime()
			form.value.xiabanshijian = context?.$toolUtil.getCurDateTime()
		if(formId){
			id.value = formId
			type.value = formType
		}
		...
		//由上级字段带出不可改
		disabledForm.value.hugongxingming = true;
		banciLists.value = "早班,中班,晚班".split(',')
	}
	//初始化
	//声明父级调用
	defineExpose({
	...
	const hugongzhanghaoChange=()=>{
		context?.$http({
			url: `follow/hugong/hugongzhanghao?columnValue=` + form.value.hugongzhanghao,
			method: 'get'
		}).then(res=>{
			if(res.data.data.hugongxingming){
				form.value.hugongxingming = res.data.data.hugongxingming
			}
		})
	}
	//富文本
