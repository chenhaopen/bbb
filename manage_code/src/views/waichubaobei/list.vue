	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
				<el-form :model="searchQuery" class="search_form" >
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.jiashuxingming" placeholder="家属姓名"
								clearable>
							</el-input>
						</div>
					</div>
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('waichubaobei','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('waichubaobei','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('waichubaobei','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('waichubaobei','删除')">删除</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column label="审核" v-if="btnAuth('waichubaobei','审核')" :resizable='true' :sortable='false' align="left" header-align="left">
					<template #default="scope">
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='false' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('waichubaobei','查看')" @click="infoClick(scope.row.id)">详情</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
...
<script setup>
	...
	const getList = () => {
		...
		params['sort'] = 'id'
		params['order'] = 'desc'
		if(searchQuery.value.jiashuxingming&&searchQuery.value.jiashuxingming!=''){
			params['jiashuxingming'] = '%' + searchQuery.value.jiashuxingming + '%'
		}
	...
	const approvalClick = (row) => {
		let params = {
			...
			waichuyuanyin: row.waichuyuanyin,
			waichushijian: row.waichushijian,
			sfsh: row.sfsh,
			shhf: row.shhf,
		}
		nextTick(() => {
