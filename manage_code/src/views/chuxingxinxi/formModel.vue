	...
	<div>
		<el-dialog v-model="formVisible" :title="formTitle" width="80%" destroy-on-close :fullscreen='false'>
			<el-form class="formModel_form" ref="formRef" :model="form" label-width="$template2.back.add.form.base.labelWidth" :rules="rules">
				<el-row>
					<el-col :span="12">
						<el-form-item label="出行日期" prop="chuxingriqi">
							<el-date-picker
								class="list_date"
								v-model="form.chuxingriqi"
								format="YYYY 年 MM 月 DD 日"
								value-format="YYYY-MM-DD"
								type="datetime"
								:readonly="!isAdd||disabledForm.chuxingriqi?true:false"
								placeholder="请选择出行日期" />
						</el-form-item>
					</el-col>
					<el-col :span="12">
					...
					<el-col :span="12">
						<el-form-item label="家属账号" prop="jiashuzhanghao">
							<el-select
								...
								placeholder="请选择家属账号"
								@change="jiashuzhanghaoChange">
								<el-option v-for="(item,index) in jiashuzhanghaoLists" :label="item" :value="item">
								</el-option>
							</el-select>
						</el-form-item>
					</el-col>
...
<script setup>
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		...
		if(formType == 'add'){
			...
			for(let x in row){
				if(x=='chuxingriqi'){
					form.value.chuxingriqi = row[x];
					disabledForm.value.chuxingriqi = true;
					continue;
				}
				if(x=='chuxingfangshi'){
	...
	const jiashuzhanghaoChange=()=>{
		context?.$http({
			url: `follow/jiashu/jiashuzhanghao?columnValue=` + form.value.jiashuzhanghao,
			method: 'get'
		}).then(res=>{
