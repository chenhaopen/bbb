		...
		<el-dialog v-model="formVisible" :title="formTitle" width="80%" destroy-on-close :fullscreen='false'>
			<el-form class="formModel_form" ref="formRef" :model="form" label-width="$template2.back.add.form.base.labelWidth" :rules="rules">
				<el-row>
					...
					<el-col :span="12">
						<el-form-item label="剂型" prop="jixing">
							<el-select
								class="list_sel"
								:disabled="!isAdd||disabledForm.jixing?true:false"
								v-model="form.jixing" 
								placeholder="请选择剂型"
								>
								<el-option v-for="(item,index) in jixingLists" :label="item"
									:value="item"
									>
								</el-option>
							</el-select>
					...
					<el-col :span="12">
						<el-form-item label="药品说明" prop="yaopinshuoming">
							<editor :value="form.yaopinshuoming" placeholder="请输入药品说明" :readonly="!isAdd||disabledForm.yaopinshuoming?true:false"
								class="list_editor" @change="(e)=>editorChange(e,'yaopinshuoming')"></editor>
						</el-form-item>
					</el-col>
				</el-row>
			</el-form>
...
<script setup>
	...
	const getInfo = ()=>{
		context?.$http({
			...
			res.data.data.yaopinshuoming = res.data.data.yaopinshuoming?(res.data.data.yaopinshuoming.replace(reg,'../../../cl32683157/file')):'';
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		...
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='jixing'){
					form.value.jixing = row[x];
					disabledForm.value.jixing = true;
					continue;
				}
				if(x=='guige'){
				...
				if(x=='yaopinshuoming'){
					form.value.yaopinshuoming = row[x];
					disabledForm.value.yaopinshuoming = true;
					continue;
				}
			}
			if(row){
		...
		jixingLists.value = "颗粒,胶囊,注射剂,丸剂,口服液体剂,贴剂,喷剂".split(',')
	}
	//初始化
	//声明父级调用
	defineExpose({
