		...
		<el-dialog v-model="formVisible" :title="formTitle" width="80%" destroy-on-close :fullscreen='false'>
			<el-form class="formModel_form" ref="formRef" :model="form" label-width="$template2.back.add.form.base.labelWidth" :rules="rules">
				<el-row>
					...
					<el-col :span="12">
						<el-form-item label="餐次" prop="canci">
							<el-select
								class="list_sel"
								:disabled="!isAdd||disabledForm.canci?true:false"
								v-model="form.canci" 
								placeholder="请选择餐次"
								>
								<el-option v-for="(item,index) in canciLists" :label="item"
									:value="item"
									>
								</el-option>
							</el-select>
					...
					<el-col :span="12">
						<el-form-item label="菜品" prop="caipin">
							<el-input class="list_inp" v-model="form.caipin" placeholder="菜品"
								 type="text" 								:readonly="!isAdd||disabledForm.caipin?true:false" />
						</el-form-item>
					</el-col>
					...
					<el-col :span="12">
						<el-form-item label="烹饪方法" prop="pengrenfangfa">
							<el-input class="list_inp" v-model="form.pengrenfangfa" placeholder="烹饪方法"
								 type="text" 								:readonly="!isAdd||disabledForm.pengrenfangfa?true:false" />
						</el-form-item>
					</el-col>
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		...
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='canci'){
					form.value.canci = row[x];
					disabledForm.value.canci = true;
					continue;
				}
				if(x=='caipin'){
					form.value.caipin = row[x];
					disabledForm.value.caipin = true;
					continue;
				}
				...
				if(x=='pengrenfangfa'){
					form.value.pengrenfangfa = row[x];
					disabledForm.value.pengrenfangfa = true;
					continue;
				}
				if(x=='jiashuzhanghao'){
		...
		context?.$http({
			...
			if(json.hasOwnProperty('jiashuxingming')&& context?.$toolUtil.storageGet("role")!="管理员"){
				form.value.jiashuxingming = json.jiashuxingming
				disabledForm.value.jiashuxingming = true;
			}
		})
		canciLists.value = "早餐,中餐,晚餐,加餐".split(',')
	}
	//初始化
	//声明父级调用
	defineExpose({
